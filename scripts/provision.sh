...
# of the gcounter peers
declare -a peers=()
declare -a peer_id_list=()

# Name of the GCounter cluster network
# connecting the peers
network="gcounter_network"
...
echo "Provisioning GCounter Docker Cluster"

echo "Building GCounter Docker Image"
docker build -t gcounter -f Dockerfile .

if [[ $? -ne 0 ]]; then
    echo "Unable To Build GCounter Docker Image"
    exit 255
fi

echo "Building GCounter Cluster Network"
...
for peer_index in "${!peers[@]}"; do
    docker run -p "${peers[$peer_index]}":8080 --net $network -e "PEERS="$comma_separated_peer_id_list"" -e "MY_NODE="${peers[$peer_index]}"" -e "NETWORK="$network"" --name="peer-$peer_index" -d gcounter
done
